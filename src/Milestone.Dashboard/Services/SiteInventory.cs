...
public static class SiteInventory
{
    ...
    private static SiteInfo Build(IGrouping<string, CameraInfo> group)
    {
        ...
        var outdated = cameras.Count(FirmwareInventory.IsOutdated);
        var current = cameras.Count(FirmwareInventory.IsCompliant);
    ...
    private static string? GetProperty(CameraInfo camera, string key) =>
        camera.CustomProperties is not null && camera.CustomProperties.TryGetValue(key, out var value) ? value : null;
